...
const Tresseme = () => {
  ...
  const tressemeProducts = products?.filter((p) => p.brands === "Tresseme");

  return (
    <div>
      <h4 className="text-lg py-8 bg-dark-200 text-center">Tresseme</h4>
      {isLoading ? (
        <p>loading...</p>
      ) : (
        <>
          {tressemeProducts?.length > 0 ? (
            <div className="my-6 grid grid-cols-1 justify-center md:grid-cols-3 lg:grid-cols-4 mx-auto items-center md:gap-4 lg:gap-2">
              {tressemeProducts?.map((p) => (
                <div key={p._id}>

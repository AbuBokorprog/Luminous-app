...
const Dandruff = () => {
  ...
  const dandruffProducts = products?.filter((p) =>
    p.concern.some((sub) => sub === "Dandruff")
  );
  return (
    <div>
      <h4 className="text-lg py-8 bg-dark-200 text-center">Dandruff</h4>
      {isLoading ? (
        <p>loading...</p>
      ) : (
        <>
          {dandruffProducts?.length > 0 ? (
            <div className="my-6 grid grid-cols-1 justify-center md:grid-cols-3 lg:grid-cols-4 mx-auto items-center md:gap-4 lg:gap-2">
              {dandruffProducts?.map((p) => (
                <div key={p._id}>
